...
if [[ "$readSteamAccount" ]]; then
    echo "screen -mS fof ${gameInstallPath}/srcds_run +ip ${ServerIP} +port ${ServerPort} -console -game fof -autoupdate +sv_pure 0 +map fof_robertlee_12 +maxplayers 24 +exec server.cfg +sv_setsteamaccount ${readSteamAccount} -steam_dir ${installPath} -steamcmd_script ${steamcmdFile}" > "${installPath}/start${GAME_SWITCHER}.sh"
