...
public Action:SoundHook(clients[64], &numClients, String:sound[PLATFORM_MAX_PATH], &Ent, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (Status[client] == RobotStatus_Robot)
	{
		...
		Internal_War3_AddSound(sound,1,PRIORITY_LOW);
		return Plugin_Changed;
	}
...
public War3Source_Engine_Cyborg_OnAddSound(sound_priority)
{
	...
	{
		Internal_War3_AddSound("mvm/sentrybuster/mvm_sentrybuster_explode.wav",STOCK_SOUND);
		Internal_War3_AddSound("mvm/sentrybuster/mvm_sentrybuster_intro.wav",STOCK_SOUND);
		Internal_War3_AddSound("mvm/sentrybuster/mvm_sentrybuster_loop.wav",STOCK_SOUND);
		Internal_War3_AddSound("mvm/sentrybuster/mvm_sentrybuster_spin.wav",STOCK_SOUND);
		for (new i = 1; i <= 18; i++)
		{
			decl String:snd[PLATFORM_MAX_PATH];
			Format(snd, sizeof(snd), "mvm/player/footsteps/robostep_%s%i.wav", (i < 10) ? "0" : "", i);
			Internal_War3_AddSound(snd,1);
			if (i <= 4)
			{
				Format(snd, sizeof(snd), "mvm/sentrybuster/mvm_sentrybuster_step_0%i.wav", i);
				Internal_War3_AddSound(snd,STOCK_SOUND);
			}
			if (i <= 6)
			{
