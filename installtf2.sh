...
if [[ "$readSteamAccount" ]]; then
    echo "screen -mS tf2 ${gameInstallPath}/srcds_run +ip ${ServerIP} +port ${ServerPort} -autoupdate +sv_consistency 0 +sv_pure 0 +map koth_nucleus +maxplayers 32 +exec server.cfg +sv_setsteamaccount ${readSteamAccount} -steam_dir ${installPath} -steamcmd_script ${steamcmdFile}" > "${installPath}/start${GAME_SWITCHER}.sh"
